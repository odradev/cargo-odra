...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let msg = match self {
            ...
            Error::FailedToReadCargo(error) => { format!("Failed to read Cargo.toml: {}", error) }
            Error::NoBackendConfigured => { "No backend configured".to_string() }
            Error::NoSuchBackend => { "No such backend".to_string() }
            Error::WasmTargetNotInstalled => { "wasm32-unknown-unknown target is not present, install it by executing:\n\
            rustup target add wasm32-unknown-unknown".to_string()}
            Error::NotAnOdraProject => { "This command can be executed only in folder with Odra project.".to_string() }
        };
