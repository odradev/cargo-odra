...
pub(crate) fn build_backend(backend: &String) {
    ...
    Command::new("cargo")
        ...
        .args(vec!["build"])
        .output().expect("Couldn't build backend");
    println!("Copying lib...");
    fs::create_dir_all("./target/debug").unwrap();
    Command::new("cp")
        .args(vec![".backend/test_env/target/debug/libodra_test_env.so", "./target/debug/libodra_test_env.so"])
        .output().expect("Couldn't copy lib");
}