...
pub fn odra_project_dependency(
    ...
    Dependency::Detailed(Box::new(DependencyDetail {
        version: version.map(|version| {
            VersionReq::parse(&version).unwrap_or_else(|err| {
                Error::FailedToReadCargo(format!("invalid Odra version `{version}`: {err}"))
                    .print_and_die()
            })
        }),
        registry: None,
...
/// Renders a version requirement the way it was most likely written in `Cargo.toml`.
///
/// `cargo_toml` parses `version = "2.9.1"` into the requirement `^2.9.1`. Odra's version is used
/// to build branch names such as `release/2.9.1`, so the caret has to go and the version has to
/// keep as many components as it was written with.
pub fn version_req_to_string(req: &VersionReq) -> String {
    match req.comparators.as_slice() {
        [comparator] => match (comparator.minor, comparator.patch) {
...
"#;
    ...
    #[test]
    fn renders_version_requirements_without_the_caret() {
        // What `version = "2.9.1"` in a generated Cargo.toml parses into.
        let cases = [
            ("2.9.1", "2.9.1"),
            ("2.9", "2.9"),
            ("2", "2"),
        ...
        for (input, expected) in cases {
            ...
            assert_eq!(version_req_to_string(&req), expected, "input: {input}");
        }
    }

    #[test]
    fn falls_back_to_the_requirement_for_compound_ranges() {
        let req = VersionReq::parse(">=2.9, <3").unwrap();
        assert_eq!(version_req_to_string(&req), req.to_string());
    }
